...
<p><em>结构化 7 阶段学习路径，从「LLM 是什么、token 怎么算」一路到 multi-agent 编排、本地部署，<br/>每阶段都有必跑 demo、必修阅读、精选 project</em></p>
...
本 repo 把网上散落各处的高质量项目、教材、Hello-X demo、必修阅读收集起来，按“从零开始、循序渐进”的顺序整理成 7 个阶段——每阶段都会清楚指出“该学什么、必跑哪些 demo、推荐哪几个 project、进入下一阶段前该检查什么”。
...
- 🛠️ **必跑 Hello-X demos** — 每阶段都有 1-5 个必跑 mini project，光看不练不算学会
...
| Stage | 主题 | 关键内容 | 预估时程 |
|---|---|---|---|
| **3** ⭐ | Tool Use & Hello Agent | function calling · ReAct · 5 个 Hello-X | 2-3 周 |
...
最重要的说一句话：**不要跳过 Hello-X demos**。每个 stage 的 Hello-X 都是“不动手就学不会”的东西，光读过去后面会卡住。
...
对, 应的 Hello-X 练习都在 [Stage 5](stages/05-claude-code-ecosystem.zh-CN.md)，Track A 的 [A3](tracks/cli/A3-cli-production.zh-CN.md) 也会用到。
...
> 上面只是 highlight。**完整 41 个集成**（含数据库、浏览器自动化、Figma、Excalidraw、Cloudflare、Stripe…）：[`resources/mcp-skills-catalog.zh-CN.md`](resources/mcp-skills-catalog.zh-CN.md)。
