...
> 📌 **最新 Claude 发布（2026-06-09）**：目前最新的 Claude 发布是 **Claude Fable 5**（`claude-fable-5`，Mythos-class 层级、定位在 Opus 之上、广泛可用的最高能力成员）；同日另发 limited-availability 的 Claude Mythos 5（`claude-mythos-5`）。Opus 4.8 仍是 Opus-class 旗舰、也是 Fable 5 的 safeguard fallback，**Dynamic Workflows 仍是 Opus 4.8 的功能**，不归属于 Fable 5。
