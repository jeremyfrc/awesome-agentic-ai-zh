...
> 💡 此阶段术语密度高（**RAG / 向量数据库 / embedding / chunking / hybrid search / reranking⋯**）→ 不熟悉先查 [`resources/glossary.md` §3](../resources/glossary.md#3-memory--retrieval--rag)。

> 📋 **本章组成**: 定位 → 入口 → **RAG 主轴**（基础 + 进阶 + DSPy + Eval）→ **Bridge** → **Memory 主轴**（3 种模式 + Trio + 进阶）→ Chunking → Reflexion / Reasoning → 练习 → Projects
>
...
| **Embedding** | 将文本转换为 N 维**向量**、使其可计算相似度（数据转换） | `sentence-transformers` 跑出 768 维向量 / OpenAI ada-002 |
| **Vector DB** | 存储 + 查询 embedding 的**存储层**（基础设施） | Chroma / Qdrant / Weaviate / pgvector |
| **RAG** | 「retrieve 相关片段 → 塞进 prompt → 生成」这个 **模式**（架构模式） | LlamaIndex / LangChain RAG chain |

→ **核心区分**: Memory 是**能力**、Embedding 是**数据转换**、Vector DB 是**存储**、RAG 是**架构模式**—这 4 个常被混用、实际上是 4 个不同层级的概念。

### RAG vs Long Context vs Fine-tuning — 何时使用什么

LLM 了解你的私有 / 领域数据有 3 种主要做法。**本阶段教授 RAG**，但你需要知道何时不该用：

| 选择 | 适合 | 不适合 | 成本 |
|---|---|---|---|
| **RAG**<br>（外部检索） | 大型 / 变动 / 私有知识库、需要 citation 引用来源 | 推理需要全文一起看的任务、需要跨文档 multi-hop reasoning | 每次查询多一次 vector search 的延迟 |
| **Long Context**<br>（直接塞入 prompt） | < 200k token 的中型文档、一次性查询、需要 cross-doc reasoning | 知识库大 / 经常变动 / 想要 citation | 每次查询消耗大量 input token（即使有 prompt caching）|
| **Fine-tuning**<br>（修改模型权重） | 风格 / 格式统一、特定领域语言（医疗、法律、代码） | 知识会变动、需要 citation、不想训练模型 | 训练成本 + 维护成本 + 模型锁定 |

→ **如何选择**: 先尝试 RAG（成本最低、变更最容易）→ RAG 找不到才考虑 Long Context → 两者都不行才考虑 Fine-tuning。**进 Stage 7 学习 fine-tune 部署**。

## 📌 学习目标

- 构建一条基础 RAG 流水线（chunk → embed → store → retrieve → generate）
- 识别 RAG 不适用的场景（以及适用的场景）
- 区分 short-term、long-term、episodic、semantic memory
- 理解 vector embedding 与相似度搜索
- 了解进阶 RAG（GraphRAG / Contextual Retrieval / Hybrid Search）何时添加、何时不添加
...
-   **Retrieve 期间**（控制流程）—— Self-improving RAG
-   **索引结构** —— RAPTOR
...
### 🎯 Hybrid Search & Reranking — Production RAG 的两个优化手段
...
**心智模型**: 上述所有 RAG 技巧都假设了一个固定的 Pipeline：“query → retrieve → generate”。Self-improving RAG 将这个 Pipeline 变成了一个**具有判断能力的 agent loop**——LLM 自己决定是否需要 retrieve，评估 retrieve 的质量，并在必要时调整查询。**这是 2024 年 RAG 研究的主轴**。
...
3.  **🤖 Agentic RAG** — Retrieval 从固定 Pipeline 演变为 Agent Loop 内的 Tool（Agent 自主决定检索次数/方式）。代表：A-RAG、Self-RAG（已在上面 §Self-improving RAG 中介绍）。
...
> 📺 **视觉学习**: [李宏毅 2025 第二讲 — 一堂课搞懂 AI Agent 的原理（含 read / write / reflection memory 模块）](https://www.youtube.com/watch?v=M2Yg1kwPpts)（NTU 生成式AI时代下的机器学习 2025）

### 短期 vs 长期 Memory — 先建立心智模型

| 比较维度 | Short-term memory（短期记忆） | Long-term memory（长期记忆） |
|---|---|---|
| **中文称呼** | 短期记忆 | 长期记忆 |
| **来源** | 当前对话内容 | 跨 session 或长期保存的信息 |
| **持续时间** | 短，通常局限于当前 session | 长，可以跨 session |
| **技术基础** | 上下文窗口（context window）/ prompt | 记忆存储层（memory store）/ 用户文件 / 向量数据库 |
| **适合记忆什么** | 任务细节、刚刚说过的内容 | 稳定的偏好、长期目标、背景信息 |
| **受上下文长度限制吗？** | 是，因为模型一次能看到的长度有限 | 较少，因为可以先存储在外部，需要时再取一小段放回上下文 |
| **生活中的例子** | 刚收到的手机验证码、正在进行的对话的上一句话 | 你深入学会的知识、图书馆、知识库、读过的书 |

这里的工作会话（session）可以理解为一次连续的交互，例如同一段聊天、同一次任务，或同一次 agent 执行。

### 3 种设计模式（何时使用哪种）⭐ Track B 必看

**并非所有 agent 都需要外部 memory store。选择错误的 memory 架构可能会导致使用十倍的 token 来达到同样的效果。**

这是开始练习前需要建立的心智模型——下面的练习 1-5 模拟的是“模式 3：Vector Store”，但 production 中你可能不需要这么复杂。

| 模式 | 适合场景 | 如何运行 | 成本 |
|---|---|---|---|
| **1. Naive buffer**<br>（全部塞入 Context） | 短对话，≤ 10 回合，agent 不需要跨 session 记忆 | 每次都将整个对话历史送入 prompt | 线性增长，token 消耗快 |
| **2. Summary + Recent**<br>（摘要旧的部分 + 保留近 N 轮） | 中长对话，约 50 回合，想压缩历史但又不想丢失太多 | 每 N 轮调用 LLM 将旧历史摘要成一段；Prompt = `summary + last N turns` | 中等，有 LLM 摘要成本 |
| **3. Vector store + retrieval**<br>（外部存储 + 每次进行 semantic search） | 跨 session 交互、知识库场景、agent 需要“想起”久远的事情 | 对过往消息进行 embed → 存入 Vector DB → 每轮查询相关片段并拼接到 prompt 中 | 高（向量计算 + 存储），但 token 使用量稳定 |

**如何选择**:

-   对话式聊天机器人，无跨 session 需求 → **模式 1**
-   Agent 配合长对话，需要记住当天聊过的内容 → **模式 2**
-   Agent 具备跨 session 需求 + 知识库（本阶段练习场景）→ **模式 3**
-   大型 production agent → 通常会**混合使用**：近期对话使用模式 1/2，长期记忆使用模式 3。

> 💡 **Track B 重点**: 你在 Stage 7 构建 multi-agent 系统时，每个 agent 通常会有“自己的 memory”+“共享 memory”两层——所需模式通常是 **2 + 3 的混合使用**。在本阶段彻底掌握这 3 种模式，到 Stage 7 才不会被 multi-agent memory 设计卡住。

### ⭐ 5 个主流 Production Memory Layer（按 Use Case 选择）

学完 3 种模式后，在 production 中不必从头编写 memory store。以下是 5 个 Apache-2.0 / MIT 许可、活跃维护、各有所长的框架：

| Framework | Stars | License | 主要 Use Case | 特色 |
|---|---|---|---|---|
| [**agentmemory**](https://github.com/rohitg00/agentmemory) | 7.7k★ | Apache-2.0 | **Coding agent 跨 session 记忆** | MCP-universal（可集成 Claude Code, Cursor, Gemini CLI, Codex, Hermes, OpenClaw 等）、95.2% R@5、92% token saving、51 个 MCP tools + 12 个 auto hooks、基于 benchmark |
| [**mem0**](https://github.com/mem0ai/mem0) | 55.6k★ | Apache-2.0 | **Chatbot / 个人助手 user-level memory** | 自动提取事实 + 遗忘 + 命名空间、经过 production 测试、社区最大 |
| [**Letta**](https://github.com/letta-ai/letta)（原 MemGPT）| 22.7k★ | Apache-2.0 | **长 session agent**（以月为单位）| OS-style 分页 memory（working + archival 双层）、persona 稳定性、MemGPT 论文起源 |
| [**Zep**](https://github.com/getzep/zep) | 4.6k★ | Apache-2.0 | **Temporal KG-based memory** | 将对话历史构建成 temporal knowledge graph，支持时间感知推理和 audit trail |
| [**LangMem**](https://github.com/langchain-ai/langmem) | 1.4k★ | MIT | **LangChain-native memory** | LangChain 官方 memory 库，直接与 LangGraph 集成，适合已采用 LangChain stack 的项目 |

**如何选择**:
-   构建 coding agent → **agentmemory**（MCP-native，与 Stage 5 ecosystem 完美对齐）
-   开发 chatbot / 个人助手 → **mem0**（最成熟、社区最大）
-   创建长运行 agent（数周/数月）→ **Letta**（OS-paging 优势明显）
-   需要时间序列数据 + audit trail → **Zep**（temporal KG）
-   已采用 LangChain stack → **LangMem**（避免框架切换）

**其他官方文档**: [Anthropic Memory Tool](https://docs.claude.com/en/docs/agents-and-tools/tool-use/memory-tool)（Claude 官方 tool-based memory，基于文件，可直接 API 调用）、[LangChain Memory concepts](https://python.langchain.com/docs/concepts/memory/)（框架内各 memory class 对比）。

### 进阶：CoALA Framework — Agent Memory 的 4 层分类法

[**Sumers et al. 2023 — Cognitive Architectures for Language Agents**](https://arxiv.org/abs/2309.02427) 将 agent memory 划分为 4 种类型，这是当前最常用的心智模型：

| 类型 | 存储内容 | 对应示例 |
|---|---|---|
| **Working memory** | 当前任务上下文 | LLM context window 本身 |
| **Episodic memory** | 过去任务的具体经验 | Reflexion 的反思记录、past trajectories |
| **Semantic memory** | 抽象事实 / 知识 | RAG 知识库、用户画像、偏好 |
| **Procedural memory** | 如何执行操作 / 技能的程序 | tool definitions、[Skills（Stage 5.3）](05-claude-code-ecosystem.md#53--skillsclaude-code-的行為層-claude-code-生態最關鍵的一層) |

→ **为何有用**：上面讨论的 3 种模式（buffer / summary / vector）主要处理 working + episodic memory。Production agent 需要设计所有 4 层——CoALA 是一个检查表，帮助你识别 agent 可能缺少了哪一层。
...
最终得分 = `α·importance + β·recency + γ·relevance`，按得分排序检索 top-k。**这是 2024-2025 年 production memory layer（mem0 / Letta）的概念骨架**。
...
| **mem0**（见 §3 主流 memory layer）| Production memory layer，自动事实提取 + 遗忘 | [mem0ai/mem0](https://github.com/mem0ai/mem0) |
...
-   [**mem0**](https://github.com/mem0ai/mem0)（上面已列）+ [**Letta**](https://github.com/letta-ai/letta)（上面已列）— Production memory layer，可以直接作为 Reflexion 的 episodic store。

> 💡 **与 Stage 3 §反思 的分工**:
> -   想理解“反思循环如何工作、单次如何运行” → Stage 3 §反思。
> -   想理解“反思如何跨 session 累积，agent 如何从过往学习经验” → 本节。
> -   想看 production agent 内部如何使用反思（例如 Cursor / Claude Code）→ [Stage 5 §5.6 Harness Internals](05-claude-code-ecosystem.md#56--claude-code-source-解剖reference-harness-implementation-track-b-必看)。
...
| **Claude Opus 4.7** | Anthropic 2026（前身：Sonnet 4.5 / Opus 4.5）| 闭源，可控 thinking budget（API 参数），在 SWE-bench / Terminal-bench 方面领先 | [Anthropic extended thinking](https://docs.claude.com/en/docs/build-with-claude/extended-thinking) |
...
| **QwQ-32B / QvQ-72B** | Alibaba Qwen 2024-11 ~ 2026 | 开源 **Apache 2.0**，32B 在小尺寸 reasoning 仍是首选，QvQ 为视觉版本 | [QwQ blog](https://qwenlm.github.io/blog/qwq-32b-preview/) |
...
> -   **Agent 能力正成为主要卖点**——V4 / Opus 4.7 都将 Agent-as-product（SWE-bench / Terminal-bench / tool use）作为核心 benchmark，单纯的 reasoning 已不足以吸引用户。
...
| **Agent 长期记忆**（见 §3 主流 memory layer）| [agentmemory](https://github.com/rohitg00/agentmemory) / [mem0](https://github.com/mem0ai/mem0) / [Letta](https://github.com/letta-ai/letta) / [Zep](https://github.com/getzep/zep) / [LangMem](https://github.com/langchain-ai/langmem) | 详见上方 §5 个主流 production memory layer 部分。 |
...
如果以上都能够做到 → 前往 [Stage 7 — Multi-Agent · 进阶应用](07-multi-agent-production.md)。
