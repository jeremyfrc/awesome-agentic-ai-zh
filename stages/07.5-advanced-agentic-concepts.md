...
> 📌 **最新 Claude 釋出（2026-06-09）**：目前最新的 Claude 釋出是 **Claude Fable 5**（`claude-fable-5`，Mythos-class 層級、定位在 Opus 之上、廣泛可用的最高能力成員）；同日另發 limited-availability 的 Claude Mythos 5（`claude-mythos-5`）。Opus 4.8 仍是 Opus-class 旗艦、也是 Fable 5 的 safeguard fallback，**Dynamic Workflows 仍是 Opus 4.8 的功能**，不歸屬於 Fable 5。
