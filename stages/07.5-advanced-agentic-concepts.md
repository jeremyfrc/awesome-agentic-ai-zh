...
| **長 horizon 任務分解 + parallel subagent** | 大型重構 / 跨檔修改超過單一 context、要拆成 subtask 平行跑 | Stage 7 multi-agent · **Opus 4.8 Dynamic Workflows（research preview、平行 subagent harness）就是這個方向的 product 化** |
...
- **reward hacking 不是邊緣案例**：在開放任務上，top model 利用 rubric 漏洞的比率 agentic code generation 達 **75%**、creative task 達 67%。
